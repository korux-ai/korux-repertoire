...
**Usages (monitor trigger vs Spec mid-flow step):** see [USAGE.md](./USAGE.md). Both use the same Vault Config and the same UNSEEN `poll`.
...
- **Not** included: Spec step filters / NL (e.g. `from_filter`) when those live on the Spec node, not in `auth.fields`.
...
4. Bind secret to staff tool **`inbound-email-monitor`** (optional for Spec `config_ref` pin).  
5. Spec: either **monitor trigger** *or* **mid-flow step** with `general/imap` + `config_ref` (see [USAGE.md](./USAGE.md)).
